package org.fisco.bcos.encryption;
/**
 * 密钥生成：
...
public class Paillier {  
	...
	public String Encryption1(String m) {
		BigInteger temp = convertToBigInteger(m);
		return Encryption(temp).toString();
	}
	...
	public String Decryption1(String c) {
		BigInteger t = new BigInteger(c);
		return convertToString(Decryption(t)).toString();
	}
	/** 
	 * 两个密文的和
	 * @param em1 
	...
	public BigInteger convertToBigInteger(String t)
	{
		BigInteger temp = new BigInteger("0");
		BigInteger t256 = new BigInteger("256");
		char[] a = t.toCharArray();
		for(int i = 0; i < a.length; i++)
		{
			BigInteger temp1 = new BigInteger(Integer.toString(a[i]));
			BigInteger temp2 = new BigInteger("1");
			for(int j = 0; j < i; j++)
			{
				temp2 = temp2.multiply(t256);
			}
	...
	public String convertToString(BigInteger t)
	{
		BigInteger div = new BigInteger("256");
		BigInteger zero = new BigInteger("0");
		char[] a = new char[8000];
		int i = 0;
		while(!t.equals(zero))
		{
			a[i] = (char)t.mod(div).intValue();
			t = t.divide(div);
			i++;
		}	
	...
	public static void main(String[] str) {  
		...
		System.out.println(paillier.Decryption1(paillier.Encryption1(test)));
		// 实例化两个数据对象m1,m2，进行加密
