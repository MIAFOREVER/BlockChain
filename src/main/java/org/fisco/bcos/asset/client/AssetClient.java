...
public class AssetClient {
	...
	public static AssetClient client = null;

	public static AssetClient getAssetClient(){
		if(client == null)
			...
			if (!response.isEmpty()) {
				if (response.get(0).ret.compareTo(new BigInteger("0")) == 0) {
					...
					System.out.printf(" transfer success => from_asset: %s, to_asset: %s, amount: %s \n",
							response.get(0).from_account, response.get(0).to_account, response.get(0).amount);
					return 0;
				} else {
	...
	public Tuple2<List<byte[]>, List<BigInteger>> selectTransaction(String account){
		Tuple2<List<byte[]>, List<BigInteger>> response = null;
		try {
			String contractAddress = loadAssetAddr();
			Asset asset = Asset.load(contractAddress, web3j, credentials, new StaticGasProvider(gasPrice, gasLimit));
			response = asset.selectTransaction(account).send();
			if (!response.getValue1().isEmpty()) {
				System.out.printf(" select transaction success !\n");
				return response;
	...
	public Tuple2<List<byte[]>, List<BigInteger>> selectTransaction1(String account){
		Tuple2<List<byte[]>, List<BigInteger>> response = null;
		try {
			String contractAddress = loadAssetAddr();
			Asset asset = Asset.load(contractAddress, web3j, credentials, new StaticGasProvider(gasPrice, gasLimit));
			response = asset.selectTransaction1(account).send();
			if (!response.getValue1().isEmpty()) {
				System.out.printf(" select transaction success !\n");
				return response;
